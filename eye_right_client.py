...
# This renders a single rightt eye (centered on screen) and
...
scleraMap = pi3d.Texture(
    "graphics/sclera.png", mipmap=False, filter=pi3d.GL_LINEAR, blend=True
)
lidMap = pi3d.Texture(
    "graphics/lid.png", mipmap=False, filter=pi3d.GL_LINEAR, blend=True
)
...
maxDist = max(
    abs(a[0] - b[0]),
    abs(a[1] - b[1]),  # Determine distance of max
    abs(a[2] - b[2]),
    abs(a[3] - b[3]),
...
d = dx * dx + dy * dy
if d > 0:
    upperLidRegenThreshold = 0.5 / math.sqrt(d)
...
d = dx * dx + dy * dy
if d > 0:
    lowerLidRegenThreshold = 0.5 / math.sqrt(d)
...
iris = mesh_init((32, 4), (0.0, 0.5 / irisMap.iy), True, False)  ## LEFT IS 0.5, 0.5
...
def frame():
    ...
    if ruRegen or (
        abs(newUpperLidWeight - prevUpperLidWeight) >= upperLidRegenThreshold
    ):
        newUpperLidPts = points_interp(
            upperLidOpenPts, upperLidClosedPts, newUpperLidWeight
        )
        if newUpperLidWeight > prevUpperLidWeight:
            upperEyelid.re_init(
                pts=points_mesh(
                    (upperLidEdgePts, prevUpperLidPts, newUpperLidPts), 5, 0, False
                )
            )
        else:
            upperEyelid.re_init(
                pts=points_mesh(
                    (upperLidEdgePts, newUpperLidPts, prevUpperLidPts), 5, 0, False
                )
            )
    ...
    if rlRegen or (
        abs(newLowerLidWeight - prevLowerLidWeight) >= lowerLidRegenThreshold
    ):
        newLowerLidPts = points_interp(
            lowerLidOpenPts, lowerLidClosedPts, newLowerLidWeight
        )
        if newLowerLidWeight > prevLowerLidWeight:
            lowerEyelid.re_init(
                pts=points_mesh(
                    (lowerLidEdgePts, prevLowerLidPts, newLowerLidPts), 5, 0, False
                )
            )  ## LEFT IS True
        else:
            lowerEyelid.re_init(
                pts=points_mesh(
                    (lowerLidEdgePts, newLowerLidPts, prevLowerLidPts), 5, 0, False
                )
            )  ## LEFT IS True
    ...
    iris.rotateToX(curY)
    iris.rotateToY(curX - convergence)  ## LEFT IS +
    iris.draw()
    eye.rotateToX(curY)
    eye.rotateToY(curX - convergence)  ## LEFT IS +
    eye.draw()
    upperEyelid.draw()
    lowerEyelid.draw()
